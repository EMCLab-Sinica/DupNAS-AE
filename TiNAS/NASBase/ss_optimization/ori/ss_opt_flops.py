...
#from NASBase.model.shuffle_arch import MNASSubNet
...
if Settings.NAS_SETTINGS_GENERAL['ARC'] == 'mobile':
    ...
    from NASBase.model.inception_arch import MNASSubNet
...
def flops_worker(cpuid, global_settings: Settings, width_multiplier, input_resolution, layer_based_cals, subnet_config_list):
    ...
    check_per_results = []
    per_result = {'id': None, 'mem': 0, 'flops':0, 'under': False, 'cpb':None}
    ...
    for i, each_subnet_config in enumerate(subnet_config_list):        
        ...
        flops_sum = 0
        mac_count = 0
        per_result = {'id': None, 'mem': 0, 'flops': 0, 'nvm': False, 'under': False, 'cpb': None, 'TS': False}
        subnet_latency_info = {}
            ...
            if USE_TS:
                ...
                if global_settings.NAS_SETTINGS_GENERAL['MODE'] == 'pdq':
                    ...
                    if peak_after_patch > (vm_available*1024):
                        print("after tinynas, can not meet vm constraint")
                        under_mem = False
                        per_result['under'] = False
                        check_per_results.append(per_result.copy())
                        continue
                ...
                elif global_settings.NAS_SETTINGS_GENERAL['MODE'] == 'none':
                    which_over_mem, mem_usage = model_tracing(onnx_path, model_name)
                ...
                per_result['TS'] = False 
                  #which_over_mem, mem_usage = model_tracing(onnx_path, model_name) 
                  #per_result['mem'] = mem_usage['peak']
                  #if per_result['mem'] > (vm_available*1024):
                  #  per_result['under'] = False

            #     peak_mem_ori = 0
            #     mem_usage = mem_usage_all_orders[0]
                
            #     for op in len(mem_usage):
            #         if mem_usage['total'][op] > peak_mem_ori:
            #             peak_mem_ori = mem_usage['total'][op]

            #     if peak_mem_ori > (vm_available*1024):
            #         print("Over VM constraint, do not use TS")
            #         under_mem = False
            #         continue
            #     else:
            #         under_mem = True

                #total_lan, under_mem ,dup_path, est_peak_per_path, q_list_per_path = model_tracing(onnx_path, model_name)
                #if not under_mem:
        ...
        check_per_results.append(per_result.copy())
        print(f"[SUBCHECK]subnet: {per_result['id']}, peak memory: {per_result['mem']}, flops: {per_result['flops']}, under_const: {per_result['under']}, nvm: {per_result['nvm']}, cpb: {per_result['cpb']}, TS: {per_result['TS']}\n")
...
def calc_flops_for_supernet(global_settings, dataset, supernet, width_multiplier, input_resolution, supernet_block_choices, n, layer_based_cals, first_block_hard_coded):
    ...
    ts_mode = global_settings.NAS_SETTINGS_GENERAL['MODE']
    txt_name = f"{ts_mode}_w{width_multiplier}_ir{input_resolution}_check_per_supernet.txt"
