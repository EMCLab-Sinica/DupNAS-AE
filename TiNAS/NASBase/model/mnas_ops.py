...
class MBConv(nn.Module):
    ...
    def __init__(self, name_prefix, C_in, C_out, kernel_size, stride, padding, expansion_factor=4, support_skip=True, affine=True, batchnorm_epsilon=1e-5):
        ...
        self.op.add_module(name+'branch1-conv0_pw', self.conv_class(C_in, C_in*t, kernel_size=1, stride=1, padding=0, groups=1, bias=False))
        self.op.add_module(name+'branch1-bn0',self.batchnorm_class(C_in*t, affine=affine, eps=batchnorm_epsilon))
        self.op.add_module(name+'branch1-relu0',nn.ReLU(inplace=False))

        # depthwise conv with BN
        self.op.add_module(name+'branch1-conv1_dw',self.conv_class(C_in*t, C_in*t, kernel_size=kernel_size, stride=stride, padding=padding, groups=C_in*t, bias=False))
        self.op.add_module(name+'branch1-bn1',self.batchnorm_class(C_in*t, affine=affine, eps=batchnorm_epsilon))
        self.op.add_module(name+'branch1-relu1',nn.ReLU(inplace=False))
        
        # projection : pointwise (1x1) conv with BN
        self.op.add_module(name+'branch1-conv2_pw',self.conv_class(C_in*t, C_out*2, kernel_size=1, stride=1, padding=0, groups=1, bias=False))
