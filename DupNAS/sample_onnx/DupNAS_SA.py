...
parser.add_argument("--onnx", type=str, default='sample_0',
                    help="Choose an input ONNX model name or path")
parser.add_argument("--output_dir", type=str, default="./",
                    help="Directory where all exported reports/figures are saved")
...
OUTDIR = "./"  # kept for backward compatibility
OUTPUT_BASE_DIR = os.path.abspath(args.output_dir)
os.makedirs(OUTPUT_BASE_DIR, exist_ok=True)

def out_path(filename):
    """Return an output path under --output_dir."""
    return os.path.join(OUTPUT_BASE_DIR, filename)

def resolve_onnx_arg(onnx_arg):
    """Accept either a model base name or a full/relative .onnx path."""
    if onnx_arg.lower().endswith(".onnx") or os.path.sep in onnx_arg or (os.path.altsep and os.path.altsep in onnx_arg):
        return os.path.abspath(onnx_arg)
    return os.path.abspath(os.path.join(DIR_PATH, onnx_arg + ".onnx"))
...
def model_tracing(onnx_path, onnx_name):
    ...
    os.makedirs(os.path.join(OUTPUT_BASE_DIR, IFD_DIR), exist_ok=True)
    onnx.save(inferred_model, os.path.join(OUTPUT_BASE_DIR, IFD_DIR, onnx_name+"_inferred.onnx"))
    ...
    model_name = out_path(onnx_name + '_order_0') 
    ...
    # === Export node_info ===
    if EXPORT_FILE:
        with open(out_path(onnx_name + "_node_info_VM" + str(AVAILABLE_VM//1024)+".txt"), "w") as f:
            for idx, node in enumerate(node_info):
                f.write(f"--- Node {idx} ---\n")
    ...
    if EXPORT_FILE:
        # === Export data_usage ===
        with open(out_path(onnx_name + "_data_usage_VM" + str(AVAILABLE_VM//1024)+".txt"), "w") as f:
        ...
        output_file = out_path(onnx_name + "_mem_usage.txt")
...
def run_test(onnx_file_path):
    ...
    if ENABLE_PDQSEL_MODE:
        ...
        if EXPORT_FILE:
            output_file = out_path(model_name+"_pdq_config_detail_VM"+str(AVAILABLE_VM//1024)+"_goal_"+GOAL_PRIORITY+".txt")
        ...
        if EXPORT_FILE:
            output_file = out_path(model_name+"_micrograph_rep_"+str(AVAILABLE_VM//1024)+"_goal_"+GOAL_PRIORITY+".txt")
            with open(output_file, "w") as f:
                f.write(f"Full_duration:{full_duration}(s)\n")
                f.write(f"peak node list:{which_over_mem}\n"), 
        ...
        if EXPORT_FILE:
            output_file = out_path(model_name + "_tinynas_repfor2_VM" + str(AVAILABLE_VM // 1024)+ ".txt")
            with open(output_file, "w") as f:
                f.write(f" Full_duration: {full_duration}(s)\n")
                f.write(f" Peak node list:{which_over_mem}\n"), 
                f.write("-" * 40 + "\n")
...
if __name__ == "__main__":
    ...
    test_model_path = resolve_onnx_arg(MODEL_NAME)
